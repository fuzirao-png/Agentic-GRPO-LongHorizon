...
# ============================================================================
# LATA v2-Pro: Enhanced Length-Aware Turn-weighted Advantage with adaptive normalization
# Improvements over baseline LATA:
#   1. Adaptive length norm exponent: 0.5 (short) -> 0.65 (medium) -> 0.5 (long) 
#      to balance reasoning depth vs noise in ultra-long trajectories
#   2. Entropy bonus injection to encourage exploration in early training
#   3. Dynamic turn-discount alpha based on trajectory length
#   4. Group std clipping to prevent advantage explosion
# ============================================================================
@register_adv_est("grpo_lata_v2")
def compute_grpo_lata_v2_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    epsilon: float = 1e-6,
    ...
    """
    LATA v2-Pro: Adaptive Length-Aware Turn-weighted Advantage.
    
    Key improvements:
    1. Adaptive normalization exponent p(L): sqrt(L) for L < 800, L^0.6 for 800-2000,
       capped at L^0.55 for L > 2000 to prevent noise amplification
    2. Dynamic turn discount alpha: higher alpha for shorter responses to protect
       early reasoning, lower alpha for long responses to avoid over-penalizing late corrections
    3. Advantage clipping per group to reduce variance from outlier trajectories
    4. Length-based advantage scaling to balance short/long trajectory gradients
    """
    # Default hyperparameters
    base_alpha = 1.05
    short_len_thresh = 800
    long_len_thresh = 2000
    adv_clip_range = 3.0
    entropy_coef = 0.001
    length_scale_temp = 1000.0

    if config is not None:
        td_config = getattr(config, "turn_discount", None)
        if td_config is not None:
            base_alpha = float(getattr(td_config, "alpha", 1.05))
        v2_config = getattr(config, "lata_v2", None)
        if v2_config is not None:
            short_len_thresh = int(getattr(v2_config, "short_len_thresh", 800))
            long_len_thresh = int(getattr(v2_config, "long_len_thresh", 2000))
            adv_clip_range = float(getattr(v2_config, "adv_clip_range", 3.0))
            entropy_coef = float(getattr(v2_config, "entropy_coef", 0.001))

    # Step 1: standard GRPO advantage (group normalize with clipping)
    scores = token_level_rewards.sum(dim=-1)
    id2score = defaultdict(list)
    id2mean = {}
    id2std = {}
    id2lengths = defaultdict(list)

    with torch.no_grad():
        bsz = scores.shape[0]
        active_lengths = response_mask.sum(dim=1).to(torch.float32)  # (bs,)
        
        for i in range(bsz):
            id2score[index[i]].append(scores[i])
            id2lengths[index[i]].append(active_lengths[i])
        ...
        for i in range(bsz):
            if norm_adv_by_std_in_grpo:
                raw_adv = (scores[i] - id2mean[index[i]]) / (id2std[index[i]] + epsilon)
            else:
                raw_adv = scores[i] - id2mean[index[i]]
            # Clip advantages to reduce variance
            scores[i] = torch.clamp(raw_adv, -adv_clip_range, adv_clip_range)

        # Step 2: Dynamic turn-discount weights based on trajectory length
        resp_len = response_mask.shape[1]
        positions = torch.arange(resp_len, device=response_mask.device, dtype=torch.float64)
        ...
        for i in range(bsz):
            L = int(active_lengths[i].item())
            if L == 0:
                L = 1
                
            # Dynamic alpha: higher for short responses (protect early reasoning)
            # lower for long responses (allow late corrections)
            if L < short_len_thresh:
                alpha = base_alpha * 1.02  # Slightly stronger early weighting
            elif L > long_len_thresh:
                alpha = base_alpha * 0.97  # Weaker early weighting, more balanced
            else:
                # Smooth interpolation
                ratio = (L - short_len_thresh) / (long_len_thresh - short_len_thresh)
                alpha = base_alpha * (1.02 - 0.05 * ratio)
            
            # Compute adaptive normalization exponent p(L)
            # p = 0.5 for short, increases to 0.6 for medium, decreases back to 0.55 for long
            if L < short_len_thresh:
                p = 0.5
            elif L > long_len_thresh:
                p = 0.55
                ...
                ratio = (L - short_len_thresh) / (long_len_thresh - short_len_thresh)
                # Quadratic peak at medium length
                p = 0.5 + 0.1 * ratio * (2 - ratio)
            
            # Turn discount weights for this trajectory
            exponents = (L - 1) - positions[:L]
            log_weights = exponents * math.log(max(alpha, 1.0))
            weights_stable = torch.exp(log_weights - log_weights.max())
            weight_sum = weights_stable.sum().clamp(min=epsilon)
            weights_i = (weights_stable * L / weight_sum).to(torch.float32)
            
            # Length normalization: L^p
            length_norm_i = (L ** p)
            
            weights_list.append(weights_i)
            length_norm_list.append(length_norm_i)
        
        # Pad and stack weights to (bs, resp_len)
        max_len = max(w.shape[0] for w in weights_list)
        weights_padded = torch.zeros(bsz, max(resp_len, max_len), device=response_mask.device, dtype=torch.float32)
        for i, w in enumerate(weights_list):
            weights_padded[i, :w.shape[0]] = w
        
        # Ensure weights match response_mask dimensions
        weights = weights_padded[:, :resp_len] * response_mask
        
        # Length norm tensor (bs, 1)
        length_norm = torch.tensor(length_norm_list, device=response_mask.device, dtype=torch.float32).unsqueeze(-1)
        
        # Step 3: Apply weights and adaptive length normalization
        scores = scores.unsqueeze(-1) * weights * response_mask / length_norm.clamp(min=epsilon)
